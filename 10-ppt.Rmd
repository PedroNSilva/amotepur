...
\begin{equation}
\widehat V(\overline y_R)=\frac{1}{N^2}\sum_{i \in s}\sum_{j \in s}\frac{(\pi_{ij}-\pi_i\pi_j)}{\pi_{ij}}\left(\frac{y_i-\overline y_R}{\pi_i}\right)\left(\frac{y_j-\overline y_R}{\pi_j}\right)(\#eq:eqppt9) \end{equation}
...
cat("<table>",paste0("<caption>", "(#tab:tabppt1)", "Resumo de estimadores dos parâmetros média e total sob PPT", "</caption>"),"</table>", sep ="\n")
```
<center>
-------------------------------------------------------   ------------------------------------
...
$Y=\displaystyle\sum_{i\in U}y_i$                            $\widehat Y=\displaystyle\sum_{i \in s}\frac{y_i}{\pi_i}=\widehat Y_{HT}$                                                  

$V(\overline y_{HT})=\frac{1}{N^2}V(\widehat Y_{HT})$        $\widehat V(\overline y_{HT})=\frac{1}{N^2}\widehat V(\widehat Y_{HT})$
...
$V(\widehat Y_{HT})$                                         $\widehat V(\widehat Y_{HT})=\displaystyle\sum_{i \in s}\sum_{j \in s}\frac{(\pi_{ij}-\pi_i\pi_j)}{\pi_{ij}}\left(\frac{y_i}{\pi_i}\frac{y_j}{\pi_j}\right)$
...
knitr::kable(tabela, align= "ccccc",format.args= list(big.mark = '.',dec=','),
...
**(#exr:exeppt1)** Verifique que o estimador de $HT$ da média não é invariante sob transformações de locação. Isto é, se tomarmos $z_i=y_i+A$, então $\overline z_{HT}\ne\overline y_{HT}+A$ 
