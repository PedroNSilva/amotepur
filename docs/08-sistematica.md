...
\begin{equation}
Y=\displaystyle\sum_{i \in U} y_i(\#eq:eqsis1)
\end{equation} 
o total populacional da variável $y$; 

\begin{equation}
\overline Y=\frac{Y}{N}=\frac{\displaystyle\sum_{i \in U} y_i}{N}(\#eq:eqsis2)
\end{equation} 
a média populacional da variável $y$; e 

\begin{equation}
S_y^2= \frac{1}{N-1}\displaystyle\sum_{i \in U}(y_i-\overline Y)^2(\#eq:eqsis3)
\end{equation} 
a variância populacional da variável $y$.
...
Logo $\widehat{Y}_{AS}$ é não viciado para o total populacional $Y$.
...
$\widehat{N}_{AS}$ é estimador não viciado para $N$, pois: 
...
\begin{equation}
\widehat{\overline Y}_{AS}= \widehat Y_{AS}/N=\frac {1}{N}\,K\,t_r(\#eq:eqsis5)
\end{equation} 

Quando $N$ é desconhecido, um estimador alternativo para a média é: 

\begin{equation}
\overline y_{AS}= \widehat Y_{AS}/\widehat N_{AS}=K\,t_r/K\,n_r=t_r/n_r=\overline y_r=\overline y(\#eq:eqsis6)
\end{equation} 
a média amostral.
...
2. As Tabelas 8.3 e 8.5 ([@Cochran1977], pp. 211-212) mostram como a ordenação dos valores da variável de pesquisa para unidades populacionais pode afetar a eficiência de amostras sistemáticas.

3. Para populações em “ordem aleatória”, amostragem sistemática é o mesmo que amostragem aleatória simples sem reposição ([@Cochran1977], seção 8.5). 
...
\begin{equation}
\frac{V_{AS}(\overline y)}{V_{AAS}(\overline y)}=1+(n-1)\rho\,\left\{\begin{array}{lll} < 1 & \textrm{se} \,\,\,\rho<0\,, \\ = 1 & \textrm{se} \,\,\,\rho=0\,,\\ > 1 & \textrm{se} \,\,\,\rho>0\,.\end{array}\right.(\#eq:eqsis10)
\end{equation} 
