...
\begin{equation}
y_i = I(i \in A) = \displaystyle \begin{cases} 1, \text { se a unidade} \textit{ i } \text{possui o atributo de interesse} \\ 0, \text { se a unidade} \textit{ i } \text{possui outro atributo} \end{cases} (\#eq:eqpro1)
\end{equation}
...
$$
\displaystyle y_i =  \begin{cases} 1, \,\, \text {se o estudante for do sexo feminino} \\ 0, \,\, \text {em outro caso} \end{cases}
$$
...
<center>
<table>
<caption>(#tab:tabprop1)Principais resultados de contagens e proporções sob *AASC*</caption>
</table>

----------
         Parâmetro                                             Estimador ENV sob AASC                                                            
------------------------------------------------------- -----------------------------------------------------------------------------------
$\displaystyle N_A = \sum_{i \in U} y_i$                $\displaystyle\widehat N_A= N\times n_A/n = N\times p$                            
...
<table>
<caption>(#tab:tabprop2)Principais resultados de contagens e proporções sob *AAS*</caption>
</table>
...
$$
\displaystyle P(Y_k = 1) = P(y_{i_k} \text{ ter o atributo de interesse}) = \frac {N_A} {N} = p \\ P(Y_k = 0) = P(y_{i_k} \text{ ter outro atributo}) = 1 - \frac {N_A} {N} = 1-p \,\, (\#eq:eqpro19) 
$$
...
Para o estimador, $\widehat p$, da proporção de unidades com o atributo de interesse na população tem-se:

$$
\displaystyle E_{AAS}(\widehat p)=p \quad\text e\quad V_{AAS}(\widehat p)=p(1-p)\frac{N-n}{n(N-1)}=\left(\frac1n-\frac1N\right)S^2_y\,\, (\#eq:eqpro25)
$$
...
\\\\ 
\displaystyle \sum _{v=n_A}^n \binom {n}{v} \widehat p_I^v (1 - \widehat p_I)^{v-k} = \alpha / 2 (\#eq:eqpro26) 
\end{cases}
...
**(#exm:exmprop2)** Voltando ao exemplo da escola com $N=1000$ alunos, suponha que foi selecionada uma amostra aleatória simples de tamanho $n=125$ na qual foi investigado o sexo de cada aluno da amostra, sendo que 60 são do sexo feminino. Constuir um intervalo de aproximadamente 95% de confiança para a proporção de alunos do sexo feminino, utilizando os vários métodos  da linguagem R sugeridos acima.
...
Na Figura \@ref(fig:figprop1) a seguir o gráfico mostra a aproximação Normal para a seleção de 1000 amostras de tamanho $n=100$, selecionadas com reposição, de uma população de tamanho $N=5000$, onde exatamente metade das unidades tem a caracterísitica de interesse ($y_i=1$).
<center>
<div class="figure">
<img src="05-proporcoes_files/figure-html/figprop1-1.png" alt="aproximação Normal no caso de uma *AASC*" width="672" />
<p class="caption">(\#fig:figprop1)aproximação Normal no caso de uma *AASC*</p>
</div>
...
<center>
<div class="figure">
<img src="05-proporcoes_files/figure-html/figprop2-1.png" alt="Aproximação Normal no caso de uma *AAS*" width="672" />
<p class="caption">(\#fig:figprop2)Aproximação Normal no caso de uma *AAS*</p>
</div>
...
@Cochran1977 mostra uma tabela, reproduzida na Tabela \@ref(tab:tabprop3), com alguns valores mínimos do total de unidades observadas na amostra, $t_y$, onde a aproximação Normal pode se utilizada.


<center>
<table>
<caption>(#tab:tabprop3)Valores mínimos de $t_y$ para uso da aproximação Normal</caption>
</table>
...
\begin{equation} 
\displaystyle IC(p;{1-\alpha})=\left[\widehat p \mp \left(z_{\alpha/ 2}\sqrt {{pq}/n}+1/2n \right) \right] (\#eq:eqpro31) \end{equation}
Ou considerando a correção para população finita:
\begin{equation} 
\displaystyle IC(p;{1-\alpha})=\left[\widehat p \mp \left(z_{\alpha/ 2}\sqrt { \left(\frac {N-n}{N-1} \right) \frac{pq}{n}}+\frac 1 {2n} \right) \right](\#eq:eqpro32) 
\end{equation}
...
\begin{equation}
d \le z_{\alpha/2}\sqrt{\left(\frac {N-n}{N-1}\right)\frac{pq}n} \implies \\ n \ge \frac{z^2_{\alpha/2}pq} {d^2 \frac {N-1}N + \frac 1 N {z^2_{\alpha /2}pq}}\doteq \frac{z^2_{\alpha/2}pq} {d^2 + \frac 1 N {z^2_{\alpha /2}pq}} = \frac {Npq}{Nd^2 /{z^2_{\alpha /2}} + pq} (\#eq:eqpro36) 
\end{equation}
...
No caso da estimação de proporções o valor de $pq$ é limitado variando de 0 a 0,25, sendo esse valor máximo atingido quando $p=q=0,5$. O gráfigo da Figura \@ref(fig:figprop3) mostra como variam os valores de $pq$ conforme variam os valore de $p$.

<center>
<div class="figure">
<img src="05-proporcoes_files/figure-html/figprop3-1.png" alt="Variação de $pq$ em função dos valores de $p$" width="672" />
<p class="caption">(\#fig:figprop3)Variação de $pq$ em função dos valores de $p$</p>
</div>
</center>
...
$$n\ge\frac {Npq}{Nd^2 /{z^2_{\alpha /2}} + pq}$$
  ...
  A reposta para o item $a$ é que a fórmula está correta, pois, sendo a população bastante grande, $N=30.000$, pode-se desprezar o fator de correção $(N-1)/N$ como mostrado na equação \@ref(eq:eqpro36), além de se tratar de fração amostral bastante pequena, justificando o uso da aproximação Normal.
...
```r
# Calculando o tamnho da mostra, arredondando o valor de n para cima
...
Quando se tem uma indicação do intervalo que pode conter a proporção a ser estimada, como no item $c$ deste exemplo, deve-se optar pelo maior tamanho de amostra necessário para suprir as exigências de precisão. Neste caso, como a informação é que $p$ deve estar no intervalo $[0,10 - 0,30]$ deve-se selecionar uma amostra de $n=6358$ domicílios. No caso de não existir nenhuma indicação sobre o valor de $p$, como no item $b$ do exemplo, deve-se tomar $n=7275$, supondo $p=0,5$.
...
\begin{equation}
\displaystyle y_i= \begin{cases} 1, \text { se a unidade} \textit{ i } \text{pertence à}\:\,\text{ categoria $c$} \\\\ 0, \text { se a unidade}\textit{ i } \text{ pertence à}\,\:\text{ outra categoria} \end{cases}(\#eq:eqpro45)
\end{equation}
...
**(#exr:exrprop1)** Uma pesquisa foi feita para estimar a proporção de domicílios de uma pequena vila que têm, pelo menos, um morador com 65 ou mais. A vila tem 651 domicílios dos quais foram pesquisados 60, e em 11 deles havia moradores com 65 anos ou mais.
...
**(#exr:exrprop3)** Supondo que o valor da variância populacional, $S^2$, de uma determinada variável de interesse, $y$, é a mesma, qual dos planos amostrais apresentados abaixo tem maior precisão para estimar uma proporção populacional? Por que?
...
**(#exr:exrprop5)** Considere a população de $N=338$ fazendas produtoras de cana de açúcar fornecida no arquivo *fazendas.dat*, na pasta *C:\\Users\\Vermelho\\Documents\\GitHub\\amotepur\\amotepur\\Dados*. Selecione uma AAS de $n=50$ fazendas, e use esta amostra para obter a estimativa pontual, o erro padrão, o CV e o intervalo de confiança de 95%, para cada um dos seguintes parâmetros populacionais:
...
---------
  ...
  c)  Estime a proporção de domicílios ALUGADO E COM SUÍTE na região, e forneça uma estimativa do CV desta proporção estimada.
  
**(#exr:exrprop9)** Uma AAS de 400 pessoas foi retirada de uma população de 2000 pessoas e 200 delas eram favoráveis a uma determinada proposta de instalção de um novo centro recreativo na localidade.
