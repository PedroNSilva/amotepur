...
\begin{equation}
y_i = I(i \in A) = \displaystyle \begin{cases} 1, \text {se a unidade} \textit{ i } \text{possui o atributo de interesse} \\ 0, \text {se a unidade} \textit{ i } \text{não possui o atributo de interesse} \end{cases} (\#eq:eqpro1)
\end{equation}
...
**(#exm:prop1)** Seja uma escola de ensino fundamental onde se deseja estudar a composição dos estudantes por sexo. Vamos supor que a escola tenha um total de 1000 estudantes, dos quais 480 são do sexo feminino. Pode-se definir a variável $y$ de interesse como:
$$
...
**(\#qua: prop1)** Principais resultados de contagens e proporções sob *AASC*
...
**(\#qua: prop2)** Principais resultados de contagens e proporções sob *AAS*
...
\begin{equation}
\displaystyle P \left(n_A = v \right) = \frac {\binom{N_A} {v} \binom{N-N_A} {n-v}}{\binom{N} {n}} , \quad \forall v=0, 1, 2, ..., min(n;N_A) \,\, (\#eq:eqpro22b) 
\end{equation}
...
\\\\ 
\displaystyle \sum _{v=n_A}^n \binom {n}{v} \widehat p_I^v (1 - \widehat p_I)^{v-k} = \alpha / 2 (\#eq:eqpro23) 
\end{cases}
...
\\\\ 
\displaystyle \sum_ {v=n_A}^{n}\frac {\binom{N\widehat p_I}{v} \binom{N - N \widehat p_I} {n-v}} {\binom{N}{n}} = \alpha / 2 (\#eq:eqpro24) 
\end{cases}  
...
[@Cochran1977] mostra uma tabela, reproduzida na \@ref(tab: prop1) Tabela 5.1, com alguns valores mínimos do total de unidades observadas na amostra, $t_y$, onde a aproximação Normal pode se utilizada.

<center> (\#tab: prop1) Tabela 5.1 - Valores mínimos de $t_y$ para uso da aproximação Normal
...
**(#exm:prop3)** Seja uma escola com 1000 alunos distribuídos entre as 9 etapas do ensino fundamental como na tabela seguinte:

\begin{array}{l|r r r } 
\hline
\text{Etapa de ensino}	& \text {Alunos} & \text {Proporção}\\
\hline
\text{1° ano} & 110 & 0,110\\
\hline
\text{2° ano} & 108 & 0,108\\	
...
\begin{equation}
\displaystyle y_i= \begin{cases} 1, \text {se a unidade} \textit{ i } \text{pertence à categoria $c$} \\\\ 0, \text {se a unidade} \textit{ i } \text{não pertence à categoria $c$} \end{cases}(\#eq:eqpro45)
\end{equation}
...
**(#exe:prop1)** Uma pesquisa foi feita para estimar a proporção de domicílios de uma pequena vila que têm, pelo menos, um morador com 65 ou mais. A vila tem 651 domicílios dos quais foram pesquisados 60, e em 11 deles havia moradores com 65 anos ou mais.
...
**(#exe:prop2)** Numa grande cidade deseja-se estimar a proporção de habitantes que são favoráveis à instalação de uma usina térmica para geração de eletricidade numa área próxima a uma reserva biológica.
