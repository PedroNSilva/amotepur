...
\begin{equation}
y_i=\displaystyle \begin{cases} 1, \text {se a unidade}\, i \,\text{possui o atributo} \\\\ 0, \text {em outro caso} \end{cases}(\#eq:eqpro1)
\end{equation}

Dessa forma, o total populacional da variável $y$ será dado pela fórmula:

\begin{equation}
Y=\displaystyle\sum_{i=1}^{N}y_i (\#eq:eqpro2) \end{equation} expressando o número de unidades populacionais que possuem a característica de interesse.

A média populacional de $y$ definida como:

\begin{equation}
\overline {Y}=\displaystyle \frac{1}{N} \sum_ {i=1}^{N}y_i = \frac {Y}{N} (\#eq:eqpro3)
...
\begin{equation} 
S^2_y=\displaystyle \frac {1}{N-1} \left( \sum_{i=1}^{N} y^2_i- N \overline{Y}^2 \right)=\frac{1}{N-1}\left(Np-Np^2 \right)=\frac{N}{N-1}p\left(1-p\right)=\frac{N}{N-1}pq (\#eq:eqpro4)
\end{equation} onde a letra $q$ denota a proporção de unidades da população que *não* possuem a caracteristica em estudo.

A variância populacional de $y$ pode também ser definida como $\sigma^2_y=pq$. Tanto $S^2_y$ como $\sigma ^2_y$ representam a dispersão da distribuição dos valores de $y$ na população. De acordo com a situação uma ou outra definição pode ser utilizada para simplificar a manipulação algébrica de alguns resultados.
...
\begin{equation} 
CV_y=\frac {\sqrt {\sigma^2_y}} {\overline Y}=\sqrt {{\sigma^2_y} /{\overline Y^2}}=\sqrt {q/p} (\#eq:eqpro5) 
\end{equation}
...
\begin{equation}
t_y=\displaystyle \sum_{i \in\ s} y_i(\#eq:eqpro6) 
\end{equation}

Da mesma forma a proporção amostral de unidades que possuem a característica em estudo será dada pela média amostral:

\begin{equation}
\displaystyle \widehat p=\overline y= \frac {1}{n} \sum_{i \in s} y_i= \frac {t_y}{n}(\#eq:eqpro7) 
\end{equation}

Pode-se facilmente verificar que $\widehat p$ é um estimador não viciado para a proporção populacional $p$, pois:

\begin{equation}
\displaystyle E_{AASC}(\widehat p)=E_{AASC}(\overline y)= \overline Y=p(\#eq:eqpro8) 
\end{equation}

A variância da proporção amostral pode ser calculada, também, analogamente a variância da média amostral.

\begin{equation}
\displaystyle V_{AASC}(\widehat p)= \frac{\sigma^2_y}{n}=\frac{pq}{n}(\#eq:eqpro9) 
\end{equation}

A variância amostral de $y$ será dada por:

\begin{equation}
\displaystyle s^2_y = \frac {n}{n-1}\widehat p (1-\widehat p)=\frac {n}{n-1}\widehat p \widehat q (\#eq:eqpro10) \end{equation}
...
\begin{equation}
\displaystyle \widehat V_{AASC}(\widehat p)=\frac{\widehat p\widehat q}{n-1}(\#eq:eqpro11) 
\end{equation}
...
\begin{equation}
y_i=\displaystyle \begin{cases} 1,\text{ se a unidade}\, i \, \text{tem o atributo} \\\\ 0,\text{ em outro caso} \end{cases}(\#eq:eqpro12) \end{equation}
...
\begin{equation}
\displaystyle P(y_i=1)=P(y_i \text{ ter o atributo de interesse})=\frac {Y} {N}=p \\ P(y_i=0)=P(y_i \text{ não ter o atributo de interesse})=1-\frac {Y} {N}=q(\#eq:eqpro13) 
\end{equation}
...
\begin{equation}
E_{AASC}(t_y)=np{\quad\text e \quad}V_{AASC}(t_y)=npq(\#eq:eqpro14) 
\end{equation}
...
\begin{equation}
\displaystyle E_{AASC}(\widehat p)=E_{AASC}\left(\frac{t_y}{n}\right)=p  {\quad\text e \quad} V_{AASC}(\widehat p)=V_{AASC}\left(\frac{t_y}{n}\right)=\frac {pq} {n}(\#eq:eqpro15) 
\end{equation}

Outro resultado importante é que se tem a distribuição de probabilidades de $\widehat p$, pois:

\begin{equation}
P \left(\widehat p=\frac kn\right)=P(t_y=k)=\binom{n} {k}p^k q^{n-k},{\quad\quad\quad\quad \text{para todo }}k=1, 2, ..., n(\#eq:eqpro16) 
\end{equation}
...
\begin{equation}
V_{AAS}(\overline y)=\frac {N-n}{N}\frac{S_y^2}{n}(\#eq:eqpro17) 
\end{equation}

Então, no caso do estimador de uma proporção, por analogia, tem-se que:

\begin{equation}
V_{AAS}(\widehat p)=\frac {N-n}{N-1}\frac{pq}{n}(\#eq:eqpro18) 
\end{equation}

Utilizando $s_y^2$ como estimador não viciado para $S_y^2$ chega-se ao estimador para a variância de $\widehat p$:

\begin{equation}
\widehat V_{AAS}(\widehat p)=\frac {N-n}{N} \frac {\widehat p \widehat q}{n-1}(\#eq:eqpro19) 
\end{equation}

Nota-se que para populações onde o número de unidades, $N$, é suficientemente grande tem-se que $\widehat V_{AAS}(\widehat p)\approx \frac {\widehat p \widehat q}{n-1}$, resultando numa equivalência entre os desempenhos da *AAS* e da *AASC*. Intuitivamente isso ocorre porque a probabilidade de seleção repetida tende a ser muito pequena no caso de populações muito grandes.
...
\begin{equation}
y_i=\displaystyle\begin{cases} 1,\text{ se a unidade} \, i\, \text{tem o atributo} \\\\ 0,\text{ em outro caso} \end{cases}(\#eq:eqpro20) 
\end{equation}
...
\begin{equation}
P(t_y=k)=\frac {\binom{Y}{k} \binom{N-Y} {n-k}} {\binom{N}{n}}, {\quad\quad\quad\quad\quad\quad0\le k \le min (n,Y)}(\#eq:eqpro21) \end{equation}
...
\begin{equation}
E_{AAS}(t_y)=n\frac Y N =np\implies E_{AAS}(\widehat p)=p(\#eq:eqpro22) 
\end{equation}
...
\begin{equation}
V_{AAS}(t_y)=\left(\frac{N-n}{N-1}\right)npq(\#eq:eqpro23) 
\end{equation}

Consequentemente a variância do estimador da proporção $p$ será dada por:

\begin{equation}
V_{AAS}(\widehat p)=V_{AAS}(\frac{t_y}{n}) = \left(\frac{N-n}{N-1}\right)\frac {pq}{n}(\#eq:eqpro24) 
\end{equation}

Um estimador da variância de $\widehat p$ será obtido substituindo $S_y^2$ pelo seu estimador não viciado que é a variância amostral, resultando em:

\begin{equation}
\widehat V_{AAS}(\widehat p)= \left(\frac{N-n}{N}\right)\frac {\widehat p\widehat q}{n-1}(\#eq:eqpro25) 
\end{equation}
...
Para isso, no caso de *AASC*, é necessário resolver o sistema de equações seguinte determinando os valores de $\widehat p_I$ e $\widehat p_S$ que o satisfaçam:

\begin{equation}
\begin{cases}\displaystyle\sum _{k=0}^{t_y} \binom {n}{k} \widehat p_S^k (1 - \widehat p_S)^{n-k}=\frac{\alpha} {2} \\\\ \displaystyle\sum _{k=t_y}^n \binom {n}{k} \widehat p_I^k (1 - \widehat p_I)^{n-k}=\frac{\alpha} {2}\end{cases}(\#eq:eqpro26) 
\end{equation}

No caso da *AAS* o sistema a ser resolvido é baseado na distribuição Hipergeométrica como se segue:

\begin{equation} 
\begin{cases}\displaystyle\sum_ {k=0}^{t_y}\frac {\binom{N\widehat p_S}{k} \binom{N-N\widehat p_S} {n-k}}{\binom{N}{n}}=\frac {\alpha}{2} \\\\ \displaystyle \sum_ {k=t_y}^{n}\frac {\binom{N\widehat p_I}{k} \binom{N-N\widehat p_I} {n-k}} {\binom{N}{n}}={\frac {\alpha}{2}}\end{cases}(\#eq:eqpro27) 
\end{equation}
...
Na Figura \@ref(fig:normalaasc) a seguir o gráfico mostra a aproximação Normal para a seleção de 1000 amostras de tamanho $n=100$, selecionadas com reposição, de uma população de tamanho $N=5000$, onde exatamente metade das unidades tem a característica de interesse ($y_i=1$).
<center>
...
[@Cochran1977] mostra uma tabela, reproduzida na Tabela 5.1, com alguns valores mínimos do total de unidades observadas na amostra, $t_y$, onde a aproximação Normal pode ser utilizada.
...
Nas condições estabelecidas para a validade da aproximação Normal tem-se que $S_y^2 \approx \sigma^2_y=pq$, portanto, $\displaystyle V_{AAS}(\widehat p) \approx V_{AASC}(\widehat p)$, então para os dois tipos de seleção pode-se considerar o intervalo de confiança para a proporção como: 
\begin{equation} 
\displaystyle IC_{AAS}(p;1-\alpha)=\left[\widehat p-z_{\alpha/ 2}\sqrt { {pq}/n}\,\,;\,\widehat p+z_{\alpha/2}\sqrt {{pq}/n}\, \right](\#eq:eqpro29) 
\end{equation}
...
\begin{equation} 
\displaystyle IC_{AAS}(p;1-\alpha)=\left[\widehat p-z_{\alpha/ 2}\sqrt { \left(\frac{N-n}{N-1} \right)\frac{pq}{n}}\,\,;\,\widehat p+z_{\alpha/2}\sqrt { \left(\frac{N-n}{N-1} \right)\frac{pq}{n}}\,\right](\#eq:eqpro30) 
\end{equation}
...
\begin{equation} 
\displaystyle IC_{AAS}(p;1-\alpha)=\left[\widehat p-z_{\alpha/ 2}\sqrt {{pq}/n}-1/2n\,\,;\,\widehat p+z_{\alpha/2}\sqrt{{pq}/n} +1/2n \right](\#eq:eqpro31) \end{equation}
Ou considerando a correção para população finita:
\begin{equation} 
\displaystyle IC_{AAS}(p;1-\alpha)=\left[\widehat p-z_{\alpha/ 2}\sqrt { \left(\frac {N-n}{N-1} \right)\frac{pq}{n}}-\frac 1 {2n}\,\,;\,\widehat p+z_{\alpha/2}\sqrt { \left(\frac {N-n}{N-1} \right)\frac{pq}{n}}+\frac 1{2n} \right](\#eq:eqpro32) 
\end{equation}
...
\begin{equation} 
\displaystyle \widehat{IC}_{AAS}(p;1-\alpha)=\left[\widehat p-z_{\alpha/ 2}\sqrt {\frac{\widehat p \widehat q} {n-1}}-\frac 1{2n}\,\,;\,\widehat p+z_{\alpha/2}\sqrt{\frac{\widehat p \widehat q} {n-1}} +\frac 1{2n} \right](\#eq:eqpro33)
\end{equation} 

\begin{equation} 
\displaystyle \widehat{IC}_{AAS}(p;1-\alpha)=\left[\widehat p-z_{\alpha/ 2}\sqrt {\left(\frac{N-n}{N}\right)\frac {\widehat p\widehat q}{n-1}}-\frac 1{2n}\,\,;\,\widehat p+z_{\alpha/2}\sqrt {\left(\frac{N-n}{N}\right)\frac {\widehat p\widehat q}{n-1}} +\frac 1{2n} \right](\#eq:eqpro34) 
\end{equation}
