...
```{r,label="tabprop1",echo=F}

knitr::kable(NULL, align='c', caption="Principais resultados de contagens e proporções sob *AASC*")
```
...
```{r,label="tabprop2",echo=F}

knitr::kable(NULL, align='c', caption="Principais resultados de contagens e proporções sob *AAS*")
```
...
```{r,label="tabprop3",echo=F}

knitr::kable(NULL,caption="Valores mínimos de $t_y$ para uso da aproximação Normal")
```
...
```{r,label="tabprop4",echo=F}

knitr::kable(NULL, align='c', caption="Distribuição dos alunos por etapa de ensino")
```
...
**(#exr:exrprop8)** Uma amostra aleatória simples sem reposição de 290 domicílios foi selecionada em certa cidade que possui 14828 domicílios. Em cada domicílio da amostra investigou-se a condição da família que o habitava, se proprietária ou locatária, e também a existência ou não de pelos menos um quarto tipo suíte (com banheiro). Os resultados obtidos são mostrados na \@ref(tab:tabprop5).

<center>


```{r,label="tabprop5",echo=F}

knitr::kable(NULL, align='c', caption="Resultados obtidos na amostra de domicílios")
```
