...
**(#exm:filh)** Considere os dados da Tabela \@ref(tab:tabvis1) de uma população fictícia com $(N=4)$ mulheres (unidades elementares), de quem foi indagado o número de filhos tidos nascidos vivos (a nossa variável $y$).

```{r, echo=FALSE, label="tabvis1"}
...
knitr::kable(tabela, booktabs = TRUE, align= "lccccc",
       caption= "Valor da variável $y$ por unidade da população de mulheres")%>%
...
knitr::kable(tabela, booktabs = TRUE, align= "ccccc",
       caption= "Informações de cada amostra possível sob plano amostral $p1$")%>%
kable_styling(full_width = F)
```  
...
knitr::kable(tabela, booktabs = TRUE, align= "lcccc",
       caption= "Probabilidade sob $p1$ para cada valor de $t$")%>%
kable_styling(full_width = F)
```
...
A Tabela \@ref(tab:tabvis4) apresenta a distribuição de probabilidades da *Soma Amostral* para valores possíveis de $2 \times t$.

```{r, echo=FALSE, label="tabvis4"}
suppressMessages(library(kableExtra)) # Pacote usado para a tabela não ocupar a largura toda da página
x1=c("Com probabilidade $p1(s)$")
x2=c("1/6")
x3=c("2/6")
...
knitr::kable(tabela, booktabs = TRUE, align= "lcccc",
       caption= "Probabilidade sob $p1$ para cada valor de 2 x $t$")%>%
kable_styling(full_width = F)
```
...
Continuando a discussão do (#exm:filh), com a população de $(N=4)$ mulheres, de quem foi indagado o número de filhos tidos nascidos vivos ($y$), tem-se, na Tabela \@ref(tab:tabvis6), o valor da variável $y$ e  a probabilidade de inclusão ${\pi}_i$ de cada unidade da população de mulheres.   

```{r, results="asis", echo=FALSE}
cat("<table>",paste0("<caption>", "(#tab:tabvis6)", "Valor da variável $y$ e probabilidade de inclusão para cada unidade da população", "</caption>"),"</table>", sep ="\n")
```
<center>
---------
...
**(#exm:filhp2)** Considere a mesma população fictícia do exemplo anterior. Considere agora o plano amostral $p2$, que retira amostras de tamanho 2 dessa população com as probabilidades indicadas na Tabela \@ref(tab:tabvis7). 

```{r, echo=FALSE, label="tabvis7"}
  x1=c(1,2,3,4,5,6,"Total")
  x2=c("{1;2}","{1;3}","{1;4}","{2;3}", "{2;4}", "{3;4}", "_")
  x3=c("{0;0}","{0;2}","{0;1}","{0;2}", "{0;1}", "{2;1}", "_")
  x4=c(0,2,1,2,1,3,9)
...
knitr::kable(tabela, booktabs = TRUE, align= "ccccc",
       caption= "Informações de cada amostra possível sob plano amostral $p2$")
```
...
A distribuição da soma amostral $t$ sob o plano $p2$ é apresentada na Tabela \@ref(tab:tabvis8). 

```{r, echo=FALSE, label="tabvis8"}
suppressMessages(library(kableExtra)) # Pacote usado para a tabela não ocupar a largura toda da página
...
knitr::kable(tabela, booktabs = TRUE, align= "lcccc",
       caption= "Probabilidade sob $p2$ para cada valor de $t$")%>%
kable_styling(full_width = F)
```
...
Estes requerem calcular as probabilidades de inclusão na amostra. A seguir, são apresentadas na Tabela \@ref(tab:tabvis9) as probabilidades de inclusão de cada uma das unidades da população, e também os pesos amostrais correspondentes sob o plano $p2$.

```{r, results="asis", echo=FALSE}
cat("<table>",paste0("<caption>", "(#tab:tabvis9)", "Probabilidade de inclusão e peso amostral de cada unidade sob o plano amostral $p2$", "</caption>"),"</table>", sep ="\n")
```
<center>
---------
...
knitr::kable(tabela, booktabs = TRUE, align= "ccccc",
       caption= "Obtenção de estimativa sob plano amostral $p2$ para cada amostra possível") %>%
kable_styling(full_width = F)
```
...
```{r, results="asis", echo=FALSE}
cat("<table>",paste0("<caption>", "(#tab:tabvis11)", "Probabilidade de seleção sob $p1(s)$ para cada valor do estimador ponderado", "</caption>"),"</table>", sep ="\n")
```
<center>
---------
Valores possíveis de $\widehat Y_w=2t$    0   2     4    6      
--------------------------------------- ---- ----  ---- ----- 
Com probabilidade $p(s)$ sob $p1(s)$    1/6  2/6   2/6  1/6  
---------
</center>
...
```{r, results="asis", echo=FALSE}
cat("<table>",paste0("<caption>", "(#tab:tabvis12)", "Probabilidade de seleção sob $p2(s)$ para cada valor do estimador ponderado", "</caption>"),"</table>", sep ="\n")
```
<center>
---------
Valores possíveis de $\widehat Y_w$    5/3   20/7   20/7+5/3      
------------------------------------- ----- ------ ---------- 
Com probabilidade $p(s)$ sob $p2(s)$  0,30  0,40   0,30
---------
</center>

A melhor estratégia é escolhida medindo o *afastamento esperado* entre os valores possíveis do estimador e o valor do total populacional desconhecido ($Y$).  Para isso, como em ambos os casos o estimador é não viciado, usamos a *variância do estimador*. A Tabela \@ref(tab:tabvis13) mostra como pode ser calculada a variância de cada um dos estimadores sob as duas opções de plano amostral ($p1$ e $p2$).


```{r, echo=FALSE, label="tabvis13"}
...
knitr::kable(tabela, booktabs = TRUE, align= "cccccc",
       caption= "Obtenção da variância dos estimadores sob os planos amostrais $p1$ e $p2$")
```
**Conclusão**
...
```{r, echo=FALSE, label="tabvis14"}
knitr::kable(NULL,caption= "Representação de cada amostra possível pelas indicadoras") 
```
------------------------------------------------------------------------------
...
knitr::kable(tabela, align= "crrr",format.args= list(big.mark = '.',dec=','),
             caption= "Valores de variáveis de interesse para cada domicílio da população")%>%
kable_styling(full_width = F) 
```
