...
$E_p(\widehat{\theta}) = \displaystyle \sum_{s\in S} \widehat{\theta}(s) p(s) \quad$
...
$RB_p(\widehat{\theta}) = \frac{B_p(\widehat{\theta})}{\theta}$.
...
$V_p(\widehat{\theta}) = \displaystyle \sum_{s \in S} [\widehat{\theta}(s) - E_p(\widehat{\theta})]^2 p(s) \quad$
...
$DP_p(\widehat{\theta}) = [V_p(\widehat{\theta})]^{1/2} \quad$

e o *coeficiente de variação* ou *CV* do estimador, dado por:

$CV_p(\widehat{\theta}) = \frac{DP_p(\widehat{\theta})}{\theta}$.
...
$EM_p(\widehat{\theta}) = [EQM_p(\widehat{\theta})]^{1/2} \quad$ 

e

$ERM_p(\widehat{\theta}) = \frac{EM_p(\widehat{\theta})}{\theta} \quad$ 
...
$\widehat Y_w = \displaystyle \sum_{i \in s} {w_i}{y_i}$
...
$\widehat Y_w = \displaystyle \sum_{i \in s} w_i y_i = \displaystyle \sum_{i \in s}  \frac{1}{\pi_i} y_i = \displaystyle \sum_{i \in s} {\pi_i}^{-1} y_i \quad$  
...
$\widehat Y_w = \displaystyle \sum_{i \in s} w_i y_i = \sum_{i \in U} w_i \delta_i y_i$,
...
$\widehat Y_{HT} = \displaystyle \sum_{i \in s} {d_i}{y_i} = \displaystyle \sum_{i \in s} \frac{y_i}{\pi_i} = \sum_{i \in s} {\pi_i}^{-1} y_i \Rightarrow\quad$ 
...
$\widehat V_1(\widehat Y_{HT}) = \displaystyle \sum_{i \in s} \sum_{j \in s} \left( {d_i d_j} - {d_{ij}} \right) {y_i} {y_j}$
...
$V_{SYG}(\widehat Y_{HT}) = \displaystyle \sum_{i \in U} \sum_{j>i} (\pi_i \pi_j - \pi_{ij}) \left( \frac{y_i}{\pi_i} - \frac{y_j}{\pi_j} \right)^2$
