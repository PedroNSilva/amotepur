...
knitr::kable(quadro14_1, booktabs = TRUE, align= "lrrrrrr",
  ...
  caption = "Estimativas de totais, erros padrão e coeficientes de variação (CV) usando os estimadores HT e de regressão (GREG) para variáveis selecionadas"
  )
```
...
knitr::kable(quadro14_2, booktabs = TRUE, align= "lrrrrrr",
             format.args= list(big.mark = '.'),
  caption = "Estimativas de totais, erros padrão e coeficientes de variação (CV) usando os estimadores HT e pós-estratificação (PS-Região) para variáveis selecionadas"
  )
```
...
*Passo 2:* Para a função de calibração $F(·)$ escolhida, resolva as Expressões \@ref(eq:eqcal29) requeridas para determinar o *multiplicador de Lagrange* ${\bf  λ}$.
